...
public class ToothBrush : MonoBehaviour
{
    ...
    {
        // Solo cuenta si el objeto realmente tiene el script del diente
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        // BLINDAJE: Si el objeto no tiene el script EstadoDiente, salimos inmediatamente y no da error
        if (diente == null) return;

        if (isBrushing && isDragging)
        {
            if (diente.EstaSiendoLimpiadoPor(global::TipoHerramienta.CEPILLO))
            {
            ...
            diente.LimpiarGradual(global::TipoHerramienta.CEPILLO, velocidadLimpieza);
        }
    }
